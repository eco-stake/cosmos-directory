...
const ChainApis = (chainId, apis, previous) => {
    ...
    urlTypes.forEach(type => {
      ...
      urls.forEach(url => {
        axios.get(url + '/' + urlPath(type), { timeout: 10000 })
          .then(res => res.data)
          .then(data => {
            if(!current[type][url]) timeStamp('Adding', chainId, type, url)
            current[type][url] = blockHeight(type, url, data)
          }).catch(error => {
            if(current[type][url]) timeStamp('Removing', chainId, type, url, error.message)
  ...
  const blockHeight = (type, url, data) => {
    let height
